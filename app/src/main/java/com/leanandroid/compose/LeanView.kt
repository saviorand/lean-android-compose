...
fun LeanView(node: JSONObject, onAction: (String) -> Unit) {
    when (node.getString("t")) {
        // Drawn as a bar above the body rather than as a Material `Scaffold`.
        // `Scaffold` is a screen-level container that fills its constraints, so it
        // throws when composed inside a vertical scroller, which hands children an
        // unbounded height.
        "scaffold" -> Column {
            node.optJSONObject("bar")?.let { LeanView(it, onAction) }
